...
const GenerateImages = () => {
  ...
  <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700 '>
     ...
     <div className='flex items-center gap-3'>
      ...
      <h1 className='text-xl font-semibold '>AI Image Geneartor</h1>
     </div>
     <p className='mt-6 text-sm font-medium'>Describe Your Image</p>
     <textarea onChange={(e)=>setInput(e.target.value)} value={input} rows={4} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' placeholder='Describe What you want to see' required/> 
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96'>
       ...
       </div>
       {
        !content ? (
          ...
          <p>Enter a topic and click "Generate Image" to get the started</p>
          </div>
       </div>
        ) : (
